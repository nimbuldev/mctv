local wave = {}
wave.version = "2.0.1"
-- Modified to be slightly more robust against malformed song files
...
function wave.createOutput(out, volume, filter, throttle, clipMode)
	...
	elseif type(out) == "table" then
		if out.execAsync then
			...
			if wave._isNewSystem then
				function output.nativePlayNote(note, pitch, volume)
					out.execAsync("playsound minecraft:block.note_block." ..
						wave._newSoundMap[note] ..
						" record @a ~ ~ ~ " .. tostring(volume) .. " " .. tostring(math.pow(2, (pitch - 12) / 12)))
				end
			else
				function output.nativePlayNote(note, pitch, volume)
					out.execAsync("playsound note_block." ..
						wave._oldSoundMap[note] ..
						" @a ~ ~ ~ " .. tostring(volume) .. " " .. tostring(math.pow(2, (pitch - 12) / 12)))
				end
			end
			return output
